...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return { title: siteName ? `Blog | ${siteName}` : "Blog" };
}
...
export default async function TenantBlogPage({ params }: Props) {
  ...
  if (site.themeSettings.onboardingDraft === true) {
    ...
    if (!user) {
      ...
      redirect(
        `https://${root}/login?return=${encodeURIComponent(`https://${tenant}.${root}/blog`)}`,
      );
    }
  ...
  const section = site.homePage.sections.find((s) => s.type === "blog");
  if (!section) redirect("/");

  const title = (section.content.title as string) || "Blog";
  const subtitle = (section.content.subtitle as string) || "";
  const posts = Array.isArray(section.content.posts)
    ? (section.content.posts as { title: string; excerpt: string; imageUrl?: string; link?: string }[])
    : [];
    ...
    <SiteShell site={site}>
      <div className="min-h-[60vh] pt-20">
        {posts.length > 0 ? (
          ...
          <EmptyPageState title={title} message="Nenhum artigo publicado ainda." />
        )}
      </div>
    </SiteShell>
  );
