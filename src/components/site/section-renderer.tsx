...
export function SectionRenderer({
  ...
  if (section.type === "testimonials") {
    ...
    // ── carousel: JS auto-advance com setas e dots ──
    if (variant === "carousel") {
      return (
    ...
    // ── split: nome à esquerda, citação à direita (look editorial/corporativo) ──
          ...
          <div className={containerClass}>
            ...
            <div className="mt-6 divide-y divide-[var(--site-border)]">
              {testimonials.map((testimonial, i) => (
                <div key={`${testimonial.author}-${i}`} className="grid gap-4 py-8 md:grid-cols-[200px_1fr]">
                  <div className="flex flex-col justify-start">
                    <p className="font-semibold text-[var(--site-accent)]">{testimonial.author}</p>
                    <div className="mt-2 h-[2px] w-8" style={{ backgroundColor: "var(--site-accent)" }} />
                  </div>
                  <div>
                    <p className="text-3xl font-serif leading-none text-[var(--site-accent)] mb-2">&ldquo;</p>
                    <p className="text-base md:text-lg leading-relaxed opacity-80">{testimonial.quote}</p>
                  </div>
                </div>
              ))}
            </div>
